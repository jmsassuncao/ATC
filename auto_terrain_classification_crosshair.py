...
class GetCoordinateCrossHair(QgsMapToolEmitPoint):
    ...
    def __init__(self, canvas):
        ...
        self.rubberBandSquare = QgsRubberBand(self.mapCanvas, QgsWkbTypes.PolygonGeometry)
        self.rubberBandSquare.setColor(Qt.blue)
        self.rubberBandSquare.setWidth(5)

        # drawn rectangle settings
        ...
        # used to clear rectangles (this is a fix, added after length and height limits)
    ...
    def canvasPressEvent(self, e):
        if e.button() == Qt.LeftButton:
            ...
            if self.times_clicked == 2:
                ...
                self.firstPoint = None
                self.secondPoint = None
                self.rubberBandSquare.reset(QgsWkbTypes.LineGeometry)
            ...
            # save coordinate to variable, and see if the distance between the points is greater than the allowed
            # polygon (this is done to prevent the creation of a polygon that overlaps multiple geographic tiles)
            # IMPORTANT: the distance calculation is based on coordinate systems based on meters, in an attempt to
            # access if the coordinate system is based on angles, the loop searches for distances lower than 360 units
            # to account for -90º,90º and -180º,180º
            ...
            if self.secondPoint is not None:
                ...
                # check if distance is greater than the max 11000 meters (empirical, it's possible that it's not enough)
                # or lower than 360, which might mean the coordinate unit is based on angles, requiring limit of 0.07
                # (also empirical and changeable)
                # if true, limit it to max
                if (x_distance <= 360) and (x_distance >= -360):
                    need_change = True
                    # check to see if the diference is positive or negative
                    if x_distance > 0.07:
                        x_aux_value = self.secondPoint.x() - 0.07
                    elif x_distance < -0.07:
                        x_aux_value = self.secondPoint.x() + 0.07

                elif (x_distance > 11000) or (x_distance < -11000):
                    need_change = True
                    # check to see if the diference is positive or negative
                    if x_distance >= 0:
                ...
                if (y_distance <= 180) and (y_distance >= -180):
                    need_change = True
                    if y_distance > 0.07:
                        y_aux_value = self.secondPoint.y() - 0.07
                    elif y_distance < -0.07:
                        y_aux_value = self.secondPoint.y() + 0.07
                elif (y_distance > 11000) or (y_distance < -11000):
                    need_change = True
                    if y_distance >= 0:
            ...
            self.rubberBandSquare.reset(QgsWkbTypes.PolygonGeometry)

            # try to predict if coordinate system is in angles or meters to draw the origin square
            if ((self.firstPoint.x() <= 360) and (self.firstPoint.x() >= -360)) and (self.firstPoint.y() <= 180) and (self.firstPoint.y() >= -180):
                # create the small square that appears when a coordinate is selected (0.0003*0.0003)
                self.rubberBandSquare.addPoint(QgsPointXY(float(self.firstPoint.x() - 0.0003), float(self.firstPoint.y()) - 0.0003), False)
                self.rubberBandSquare.addPoint(QgsPointXY(float(self.firstPoint.x() + 0.0003), float(self.firstPoint.y()) - 0.0003), False)
                self.rubberBandSquare.addPoint(QgsPointXY(float(self.firstPoint.x() + 0.0003), float(self.firstPoint.y()) + 0.0003), False)
                self.rubberBandSquare.addPoint(QgsPointXY(float(self.firstPoint.x() - 0.0003), float(self.firstPoint.y()) + 0.0003), True)
            else:
                # create the small square that appears when a coordinate is selected (10*10)
                self.rubberBandSquare.addPoint(QgsPointXY(float(self.firstPoint.x() - 10), float(self.firstPoint.y()) - 10), False)
                self.rubberBandSquare.addPoint(QgsPointXY(float(self.firstPoint.x() + 10), float(self.firstPoint.y()) - 10), False)
                self.rubberBandSquare.addPoint(QgsPointXY(float(self.firstPoint.x() + 10), float(self.firstPoint.y()) + 10), False)
                self.rubberBandSquare.addPoint(QgsPointXY(float(self.firstPoint.x() - 10), float(self.firstPoint.y()) + 10), True)

            # show square
            self.rubberBandSquare.show()
